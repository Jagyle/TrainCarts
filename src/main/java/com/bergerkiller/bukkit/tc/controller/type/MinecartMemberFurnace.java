...
public class MinecartMemberFurnace extends MinecartMember<CommonMinecartFurnace> {
    ...
    @Override
    public boolean onInteractBy(HumanEntity human, net.minecraft.server.v1_9_R1.ItemStack i, EnumHand hand) {
        if (!this.isInteractable()) {
            return true;
        }
        ItemStack itemstack = human.getItemOnCursor();  //TODO fix for setiteminhand?
        if (itemstack != null && itemstack.getType() == Material.COAL) {
            ItemUtil.subtractAmount(itemstack, 1);
            human.setItemOnCursor(itemstack); //TODO fix for setiteminhand?
